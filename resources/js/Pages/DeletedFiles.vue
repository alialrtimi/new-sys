...
function restorePerson(id) {
    ElMessageBox.confirm(
        'هل أنت متأكد من أنك تريد استرجاع هذا العنصر؟',
        'تأكيد الإسترجاع',
        {
                ...
                <el-table :data="people.data" border style="width: 100%">
                    ...
                    <el-table-column prop="judgment_date" label="#" v-if="$inertia.page.props.auth.user.type != 'user'">
                        <template #default="scope">
                            ...
                            <button v-if="$inertia.page.props.auth.user.type != 'user'" class="btn btn-sm btn-success"
                                @click="restorePerson(scope.row.id)">
                                استرجاع
                            </button>
